...
public class SearchTest extends BaseApiTest {
    ...
    private final CheckedBase<Project> checkedProjectRequest = new CheckedBase<>(Specifications.getSpec()
            .superUserSpec(), PROJECTS);
    private final CheckedBase<User> checkedUserRequest = new CheckedBase<>(Specifications.getSpec()
            .superUserSpec(), USERS);
    private final CheckedBase<BuildType> checkedBuildTypeRequest = new CheckedBase<>(Specifications.getSpec()
            .superUserSpec(), BUILD_TYPES);

    @Test(description = "User should be able to search models", groups = {"Regression"})
    public void searchTest() {
        var initialProjectSize = checkedProjectRequest.search().size();
